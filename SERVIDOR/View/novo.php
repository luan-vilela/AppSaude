<!DOCTYPE html>
<html>
<head>
    <meta charset="utf-8">
    <meta http-equiv="X-UA-Compatible" content="IE=edge">
    <title>Cadastrar Contato</title>
    <link rel="stylesheet" type="text/css" href="View/css/style.css">
    <link rel="stylesheet" href="https://www.w3schools.com/w3css/4/w3.css">
    <link rel="stylesheet" href="https://cdnjs.cloudflare.com/ajax/libs/font-awesome/4.7.0/css/font-awesome.min.css">
    <meta name="viewport" content="width=device-width, initial-scale=1">

</head>
<body>
    <div class="formulario">
        <div class="conteudo">
            <form method="post" action="?function=cadastrarContato" class="w3-container w3-card-4 w3-light-grey w3-text-blue w3-margin">
                <h2 class="w3-center">Cadastrar Contato</h2>
                <input  class="w3-input w3-border" type="text" name="nome" placeholder="Nome" />
                <input  class="w3-input w3-border" type="email" name="email" placeholder="Email" />
                <input class="w3-button w3-block w3-section w3-blue w3-ripple w3-padding" type="submit" value="Enviar"/>
            </form>
        </div>
    </div>
</body>
</html>